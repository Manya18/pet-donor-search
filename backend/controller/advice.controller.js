...
class AdviceController {
  async createAdvice(req, res) {
    const { user_id, advice_text, admin_accept } = req.body;
    try {
      const newAdvice = await db.query(
        `INSERT INTO advice (user_id, advice_text, admin_accept) values ($1, $2, $3) RETURNING *;`,
        [user_id, advice_text, admin_accept]
      );
      res.json(newAdvice.rows[0]);
    } catch (e) {
      return res.status(400).json({ message: "Не удалось записать совет" });
    }
  }
